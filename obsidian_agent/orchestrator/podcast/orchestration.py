"""Podcast orchestrator using a persistent Dedalus Machine with autosleep.

Flow:
  First call:    Create machine (autosleep=30m) → setup server → generate podcast
  Later calls:   Find machine → wake if Dedalus auto-slept it → generate podcast
  Machine gone:  Fall back to create + setup

The machine_id is persisted in .machine_state.json next to this module so
the VM can be reused across processes (TUI restarts, etc.). Dedalus auto-sleeps
the VM after AUTOSLEEP_WINDOW of inactivity, so no explicit sleep call is needed.
...
def delete_all_machines() -> list[str]:
    """Destroy all non-destroyed machines in the org."""
...
def generate_podcast(note_path: str, note_content: str) -> dict:
    """Generate a podcast using a persistent Dedalus machine with autosleep.
    ...
    try:
        ...
        # 3. uvicorn runs as podcast-server.service — systemd starts it on
        ...
        if "MISSING" in check_out:
            print("App directory missing — re-running setup.sh (~12 min)...")
            _run_setup(client, machine_id, api_key)
        ...
        # 5. Save the returned audio to the vault and return metadata.
        ...
        print(f"Podcast generation failed: {exc}")
        try:
